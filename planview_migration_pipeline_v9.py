"""
============================================================
Planview Migration Pipeline v9  — Customer Rules Edition
============================================================
How it works:
...
Usage:
  python planview_migration_pipeline_v9.py
...
# ── OUTPUT FOLDER — UPDATE BEFORE RUNNING ────────────────────────────────
# Local folder where the output Excel file will be saved.
# The folder is created automatically if it does not exist.
# File is saved AND emailed — both happen every run.

OUTPUT_FOLDER  = r"C:\Demo\Outputs"   # update to your preferred folder path
...
def save_to_folder(excel_bytes, ts):
    """Save the Excel output to the configured local folder.
...
def send_email(excel_bytes, ts, final_df, stopwork_df, review_df, excluded_df):
    ...
    body = (
        ...
        "This is an automated email from the Planview Migration Pipeline v9.\n\n"
        "Regards,\nMigration Pipeline\n"
    )

    # Save Excel bytes to a temp file so Outlook can attach it
    # Outlook COM requires a real file path for attachments
    # We reuse the already-saved output file from save_to_folder()
    # to avoid writing the file twice. Fallback to temp file if needed.
    tmp_path = None
    saved_path = Path(OUTPUT_FOLDER) / f"Planview_Migration_Output_{ts}.xlsx"
    try:
        import win32com.client

        # Use the already-saved output file as the attachment
        # No need to write a second temp file
        if saved_path.exists():
            tmp_path = str(saved_path)
            _delete_after = False   # don't delete — it's the saved output
        else:
            # Fallback: write a temp file if saved file not found
        ...
        # Only delete the file if it was a temp file (not the saved output)
...
def main():
    start = datetime.now()
    print(SEPARATOR)
    print("  Planview Migration Pipeline v9 — Customer Rules Edition")
