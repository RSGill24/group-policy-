...
# ── EMAIL CONFIG (SendGrid API) — UPDATE THESE BEFORE RUNNING ────────────
# 1. Sign up free at https://sendgrid.com  (100 emails/day free)
# 2. Go to Settings → API Keys → Create API Key → Full Access
# 3. Copy the key below (starts with SG.)
# 4. Verify your sender email at:
#    Settings → Sender Authentication → Single Sender Verification

SENDGRID_API_KEY = "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"  # paste your API key here

# Sender — must be verified in SendGrid Sender Authentication
EMAIL_FROM       = "nileshp@sedatacloud.com"
EMAIL_FROM_NAME  = "Planview Migration Pipeline"

# Recipients
EMAIL_TO         = [
    "nileshp@sedatacloud.com",    # update with actual recipient
]
...
def send_email(excel_bytes, ts, final_df, stopwork_df, review_df, excluded_df):
    """Send the in-memory Excel file via SendGrid API. No SMTP, no passwords."""
    log_step("6/6", "Sending output Excel by email via SendGrid...")
    ...
    to_list = [{"email": addr} for addr in EMAIL_TO]
    cc_list = [{"email": addr} for addr in EMAIL_CC] if EMAIL_CC else []

    payload = {
        "personalizations": [{
            "to": to_list,
            **({"cc": cc_list} if cc_list else {}),
            "subject": f"Planview Migration Output - {ts}",
        }],
        "from": {
            "email": EMAIL_FROM,
        ...
        "attachments": [{
            "content":     __import__("base64").b64encode(excel_bytes).decode(),
    ...
    # Send via SendGrid REST API — no SMTP, no ports, works through any firewall
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            if resp.status == 202:
                ...
                log(f"WARNING: Unexpected response status {resp.status}", 1)

    except urllib.error.HTTPError as e:
        error_body = e.read().decode("utf-8")
        log(f"ERROR: SendGrid API returned {e.code}", 1)
        log(f"Detail : {error_body}", 1)
        if e.code == 401:
            log("- SENDGRID_API_KEY is invalid or expired.", 2)
            log("- Go to sendgrid.com → Settings → API Keys and generate a new one.", 2)
        elif e.code == 403:
            log("- Sender email not verified in SendGrid.", 2)
            log("- Go to sendgrid.com → Settings → Sender Authentication.", 2)
            log("- Verify the EMAIL_FROM address before sending.", 2)
        sys.exit(1)
    except urllib.error.URLError as e:
        log(f"ERROR: Could not reach SendGrid API: {e.reason}", 1)
        log("- Check your internet connection.", 2)
        sys.exit(1)
