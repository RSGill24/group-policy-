...
def build_excel(df_inits, df_epics, df_sba, df_tasks, df_in_raw, df_ep_raw, df_tk_raw,
    ...
    def _sanitize_df(df):
        ...
        Tests each value against openpyxl's own check_string to guarantee
        compatibility regardless of openpyxl version.
        Also truncates to Excel's 32,767 character cell limit.
        """
        from openpyxl.cell.cell import Cell
        from openpyxl.utils.exceptions import IllegalCharacterError
        import re

        # XML 1.0 legal characters as fallback cleanup
        xml_legal = re.compile(
            u'[^\u0009\u000A\u000D\u0020-\uD7FF\uE000-\uFFFD\U00010000-\U0010FFFF]'
        )
        MAX_CELL = 32767
        ...
        def clean(v):
            ...
            s = str(v)[:MAX_CELL]
            try:
                # Use openpyxl's own validator — catches version-specific rules
                Cell.check_string(None, s)
                return s
            except IllegalCharacterError:
                # Strip chars one by one keeping only those openpyxl accepts
                cleaned = []
                for ch in s:
                    try:
                        Cell.check_string(None, ch)
                        cleaned.append(ch)
                    except IllegalCharacterError:
                        pass  # drop the bad character
                return ''.join(cleaned)
